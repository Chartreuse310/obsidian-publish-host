(() => {
    ...
    f.addAll = function (A) {
        var t, n, i, g, e = A.length, I = new Array(e), o = new Array(e), a = 1 / 0, h = 1 / 0, f = -1 / 0,
            C = -1 / 0;
        for (n = 0; n < e; ++n)
    ...
    f.remove = function (A) {
        if (isNaN(g = +this._x.call(null, A)) || isNaN(e = +this._y.call(null, A)))
            return this;
        var t, n, i, r, g, e, I, o, a, h, f, C, s = this._root, u = this._x0, l = this._y0, Q = this._x1,
            B = this._y1;
        if (!s)
            return this;
        if (s.length)
    ...
    var S, p, U, _, N, q, R, k, b, J, Y
    var K = function () {
        try {
            // 浏览器支持 WebAssembly
            if ("object" == typeof WebAssembly && "function" == typeof WebAssembly.instantiate) {
                const bufferSource = function (encodedData) {
                    let rawStringData = atob(encodedData)
                    let uint8Array = new Uint8Array(rawStringData.length)
                    for (let i = 0; i < rawStringData.length; i++) {
                        uint8Array[i] = rawStringData.charCodeAt(i)
                    }
                ...
                let exports = new WebAssembly.Instance(module, {
                    env: {
                        abort: function () {
                        },
                        clog: console.log,
                ...
                return function () {
                    ...
                    if (memory.buffer.byteLength < A) {
                        var t = Math.ceil(A / 65536) - memory.buffer.byteLength / 65536;
                        memory.grow(t)
                    }
                    var r = new Float32Array(memory.buffer)
                        , g = new Int32Array(memory.buffer, 20 * M.length);
                    if (d || D) {
                        d = D = !1;
                        for (var e = 0; e < M.length; e++) {
                            var I = M[e];
                        ...
                        exports.init(0, M.length, G.length)
                    }
                    exports.simulate(0, M.length, G.length, Q, c, E, v, x, .9, .5),
                        exports.complete(0, M.length, .6);
    ...
    if (L) {
        let buffer = null
        Y = function () {
            let len = 4 * M.length * 2 + 4;
            if (!buffer || buffer.byteLength !== len) {
                buffer = new SharedArrayBuffer(len)
            }
            let id = X(buffer)
            let n = new Uint32Array(buffer, buffer.byteLength - 4, 1)
            let i = n[0];
            n[0]++
            postMessage({
                id: id,
